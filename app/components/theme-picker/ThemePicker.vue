...
const radius = computed({
    get() {
        return (appConfig.theme as any).radius
    },
    set(option) {
        (appConfig.theme as any).radius = option
        window.localStorage.setItem('nuxt-ui-radius', String((appConfig.theme as any).radius))
    }
})
...
function setBlackAsPrimary(value: boolean) {
    (appConfig.theme as any).blackAsPrimary = value
        ...
        <template #content>
            <fieldset>
                ...
                <div class="grid grid-cols-3 gap-1 -mx-2">
                    <ThemePickerButton label="Black" :selected="(appConfig.theme as any).blackAsPrimary"
                    ...
                    <ThemePickerButton v-for="color in primaryColors" :key="color" :label="color" :chip="color"
                        :selected="!(appConfig.theme as any).blackAsPrimary && primary === color" @click="primary = color" />
                </div>
            </fieldset>
            ...
            <fieldset>
                ...
                <div class="grid grid-cols-3 gap-1 -mx-2">
                    <ThemePickerButton v-for="m in modes" :key="m.label" v-bind="m"
                        :selected="colorMode.preference === m.label" @click="mode = m.label" />
                </div>
            </fieldset>
        </template>
    </UPopover>
